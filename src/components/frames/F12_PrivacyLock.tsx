...
import { Heart, Lock, BookOpen, Link2, ShieldCheck, MessageSquare, Bookmark, Users } from "lucide-react";

const blocked = [
  { Icon: Heart, t: "Emotional memories", g: "from-brand-pink to-brand-peach" },
  { Icon: Lock, t: "Private relationship notes", g: "from-brand-purple to-brand-lavender" },
...
export function F12_PrivacyLock() {
  return (
    <div className="relative w-full h-full pt-12 font-sans text-brand-ink overflow-hidden gradient-brand-soft">
      ...
      <motion.div
        ...
        className="absolute bottom-0 left-0 right-0 z-10 bg-white rounded-t-[32px] pb-6 max-h-[90%] overflow-y-auto shadow-glow"
      >
        ...
        <div className="px-6 pt-4 flex items-start gap-3">
          <div className="w-14 h-14 rounded-2xl gradient-brand flex items-center justify-center shrink-0 shadow-glow">
        ...
        <div className="px-6 mt-5 flex items-center justify-between">
          <div className="text-[10px] font-bold tracking-[0.7px] text-brand-mute">BLOCKED IN THIS PATH</div>
        ...
        <div className="mx-6 mt-3 space-y-2">
          {blocked.map(({ Icon, t, g }) => (
            ...
            >
              ...
              <Lock size={13} className="text-brand-mute"/>
            </motion.div>
          ))}
        </div>

        <div className="px-6 mt-5 text-[10px] font-bold tracking-[0.7px] text-brand-mute">USED INSTEAD</div>
        <div className="px-6 mt-3 grid grid-cols-3 gap-2">
          {[
            { Icon: MessageSquare, t: "Past chat: weekend plan" },
            { Icon: Bookmark, t: "Memory: hiking" },
        ...
        <div className="px-6 mt-6 space-y-2">
          ...
          <motion.button whileTap={{ scale: 0.98 }} className="w-full py-3.5 rounded-2xl bg-brand-bg text-brand-ink text-[13px] font-bold">
            Close
          </motion.button>
        </div>
