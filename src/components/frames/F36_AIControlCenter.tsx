...
import { Settings, Flag, Bot, Ban, Lightbulb, PenLine, Send, User, Users, Heart, ChevronRight, Plus, AlertTriangle, Mic } from "lucide-react";
    ...
    <div className="relative w-full h-full pt-12 pb-20 overflow-y-auto font-sans text-brand-ink gradient-brand-soft">
      ...
      <div className="mx-4 bg-white rounded-2xl p-3.5 flex items-center gap-3 shadow-soft border border-white">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[11px] text-brand-mute">Today's boundary task · 1/3 done</div>
        </div>
        <button className="px-3.5 py-2 rounded-full gradient-brand text-white text-[11px] font-bold shadow-soft">Continue</button>
      </div>
      ...
      <div className="mx-4 mt-4 bg-white rounded-3xl p-4 shadow-soft border border-white">
        ...
        <div className="mt-2 flex items-center justify-between">
          <div>
            <div className="text-[22px] font-bold gradient-brand bg-clip-text text-transparent">Assistant</div>
            <div className="text-[11px] text-brand-mute">Medium AI involvement</div>
          </div>
          <motion.div animate={{ rotate: 360 }} transition={{ duration: 18, repeat: Infinity, ease: "linear" }} className="relative w-16 h-16 rounded-full border-2 border-dashed border-brand-lavender flex items-center justify-center">
            <motion.div animate={{ rotate: -360 }} transition={{ duration: 18, repeat: Infinity, ease: "linear" }} className="w-12 h-12 rounded-2xl gradient-brand flex items-center justify-center shadow-glow">
              <Bot size={20} className="text-white"/>
            </motion.div>
          </motion.div>
        </div>
        <div className="mt-3 flex items-center gap-2">
          <button className="flex-1 py-2.5 rounded-2xl bg-brand-ink text-white text-[12px] font-bold">Change Mode</button>
        ...
        <div className="mt-3 flex items-center gap-2 pt-3 border-t border-brand-bg">
          ...
          <button onClick={() => setAdaptive(!adaptive)} className={`relative w-10 h-6 rounded-full transition-colors ${adaptive ? "gradient-brand" : "bg-brand-bg"}`}>
            <motion.div layout className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow ${adaptive ? "right-0.5" : "left-0.5"}`}/>
          </button>
        </div>
      </div>
      ...
      <div className="mx-4 mt-2 bg-white rounded-2xl p-3.5 shadow-soft border border-white">
        ...
        <div className="mt-3 grid grid-cols-4 gap-2">
          {modes.map((m, i) => (
            <button key={m.t} onClick={() => setMode(i)} className="flex flex-col items-center gap-1.5">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${i === mode ? "gradient-brand shadow-glow" : "bg-brand-bg"}`}>
                <m.Icon size={16} className={i === mode ? "text-white" : "text-brand-mute"}/>
              </div>
              <div className={`text-[10px] font-bold ${i === mode ? "text-brand-purple" : "text-brand-mute"}`}>{m.t}</div>
            </button>
          ))}
        </div>
      ...
      <div className="mx-4 mt-2 bg-white rounded-2xl p-3.5 shadow-soft border border-white">
        <div className="text-center text-[12px] font-bold mb-3">Complete Basic Info</div>
        <div className="divide-y divide-brand-bg">
          {[["Name", "Sabrina"], ["ID", "gakajo"], ["Personal intro", "+"]].map(([k, v]) => (
            <div key={k} className="py-2.5 flex items-center justify-between">
              <span className="text-[12px]">{k}</span>
              <span className="text-[11px] text-brand-mute flex items-center gap-1.5">{v} <ChevronRight size={12}/></span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mx-4 mt-3 bg-white rounded-2xl p-3.5 shadow-soft border border-white">
        ...
        <div className="space-y-2">
          {["Voice", "Voice self-introduction"].map((v) => (
            <button key={v} className="w-full flex items-center justify-between p-2.5 rounded-xl bg-brand-bg/50">
              <span className="text-[11px] flex items-center gap-2"><Mic size={12} className="text-brand-purple"/>{v}</span>
