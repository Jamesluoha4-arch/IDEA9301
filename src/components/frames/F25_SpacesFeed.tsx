...
import { ArrowLeft, Bell, Bot, Heart, MessageSquare, Share2, Plus, ShieldCheck, User } from "lucide-react";
...
const posts: Post[] = [
  {
    ...
    body: "Exploration of geometric shadows in brutalist architecture. AI-rendered lighting simulation focusing on the interaction between structural concrete and high-altitude solar positioning.",
    likes: "1.2K",
    comments: 48,
    ref: "REF: #772-BX",
    tags: ["AI_MATCH: 98.4%", "LENS: VIRTUAL_50MM"],
  },
  {
    user: "ERIK_S_08",
    time: "JUST NOW",
    ...
    comments: 156,
    ref: "REF: #ORIG-99",
    tags: ["CERTIFIED_ORIGINAL", "ISO: 100", "f/1.8"],
  },
];
...
function AuthBadge({ t }: { t: Post["authType"] }) {
  const ai = t === "AI_SYNTH";
  return (
    <div className={`px-2 py-0.5 rounded-md text-[8px] font-bold tracking-[0.4px] flex items-center gap-1 ${ai ? "bg-brand-purple/10 text-brand-purple" : "bg-brand-mint/15 text-brand-mint"}`}>
...
export function F25_SpacesFeed() {
  return (
    <div className="relative w-full h-full pt-12 pb-20 overflow-y-auto font-sans text-brand-ink gradient-brand-soft">
        ...
        {posts.map((p, idx) => (
          ...
          >
            <div className="px-3 pt-3 flex items-center gap-2">
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${p.authType === "AI_SYNTH" ? "gradient-brand" : "gradient-mint-sky"}`}>
                {p.authType === "AI_SYNTH" ? <Bot size={14} className="text-white"/> : <User size={14} className="text-white"/>}
              </div>
            ...
            <div className="mx-3 mt-3 h-[140px] rounded-2xl relative overflow-hidden" style={{ background: p.cover }}>
              <motion.div animate={{ x: ["-100%", "100%"] }} transition={{ duration: 3, repeat: Infinity }} className="absolute inset-y-0 w-1/3 bg-gradient-to-r from-transparent via-white/15 to-transparent"/>
            </div>
      ...
      {/* FAB */}
      <motion.button whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.95 }} className="absolute bottom-24 right-5 w-12 h-12 rounded-full gradient-brand shadow-glow flex items-center justify-center">
