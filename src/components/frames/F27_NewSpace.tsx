  ...
  return (
    <div className="relative w-full h-full pt-12 pb-6 overflow-y-auto font-sans text-brand-ink gradient-brand-soft">
      ...
      <div className="px-5 mt-2 flex items-center justify-between">
        <h1 className="text-[26px] font-bold tracking-[-0.5px]">New <span className="bg-gradient-to-r from-brand-purple to-brand-pink bg-clip-text text-transparent">Space</span></h1>
        <motion.button whileTap={{ scale: 0.96 }} whileHover={{ y: -1 }} className="px-4 py-2 rounded-full gradient-brand text-white text-[11px] font-bold tracking-[0.6px] shadow-soft">
          PUBLISH
        </motion.button>
      </div>
      ...
      <div className="px-5 mt-5">
        ...
        <motion.div whileHover={{ y: -2 }} className="mt-2 rounded-2xl border-2 border-dashed border-brand-lavender/50 bg-white/60 p-6 flex flex-col items-center justify-center gap-2">
      ...
      <div className="px-5 mt-6 text-[10px] text-brand-mute leading-[14px]">
        Your post will be reviewed by your Second Self for safety before publishing to the community.
      </div>
    </div>
  );
}
